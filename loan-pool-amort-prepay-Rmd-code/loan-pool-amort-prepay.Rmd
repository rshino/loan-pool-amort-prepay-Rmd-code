...
orig.term<-360
note.rate<-0.06     # e.g., 0.06 == 6% annual note rate, or 0.5% monthly rate (no compounding used)
orig.bal<-100000
scalar.cpr<-0.06    # flat life CPR, e.g., 0.06 == 6% conditional prepay rate which is an annual compounded rate
...
# time series of binary payment outcomes, 1.0 for paid in full and 0.0 for amortize. This series translates 
# directly to CPR (or SMM, as the 0.0 and 1.0 are equivalent CPR and SMM) vectors which are passed 
# into the amort.loan.cf() function. Each loan cashflow is aggregated into a pool cashflow. 
# The resulting pool cashflow of discrete loan prepayments more closely resembles observable pool cashflows
# than the continuous cashflows using the MBS pool convention, but converges with the MBS pool convention
# as loan count exceeds 250 or more.
