...
## Part I. Amortization and Prepayment Calculations for Fixed-Rate Level-Pay Mortgages
...
print.time.stats<-function(time,title=NA){
  knitr::kable(t(time[1:3]),
               ...
               format.args=list(width=50)) %>% kable_styling()
}
```
...
#### Effect of simulating pools with larger and smaller loan counts
...
calc.hist.cpr<-function(cf){
  ...
  # e.g., hist.cpr[m,sched.ebal01] is the sched bal projected from hist.cpr[m-1,ebal] (0 ppy)
...
# plot.comp.bals(cf,cf2,title) ===========================================================
#
# plot comparison balances
plot.comp.bals<-function(cf,cf2,title){
  par(mar=c(bottom=5, left=5, top=3, right=5) + 0.1) # bot, left, top, right in lines
  ...
  axis(1, at = seq(0, orig.term, 12*5),cex.axis=cex.size)
  axis(2, at = seq(0, maxy, maxy/4),
       prettyNum(seq(0, displ.y, displ.y/4),big.mark=",", scientific=FALSE),las=1,
       cex.axis=cex.size)
...
From loan and pool cashflows, calculate observed CPRs from balances and unscheduled principal.
...
We execute 1,000 or more random trials of loan simulation in the manner of the last example. 
We extract historical prepayments from the simulations and observe the distribution of 
1-, 3-, 6-, and 12-month CPR. As is visible in the simulation CPR graph, 
...
```{r sim.trials_f}
# sim.trials(cutoff.mo) ==================================================================
#
# run the pool simulation, extract historical CPR, return in list, suitable to be saved 
# in data.table
sim.trials<-function(cutoff.mo){
  cfsim<-simulate.loan.pool.f(orig.term,note.rate,orig.bal,scalar.cpr,loan.count)
  hist.cpr<-calc.hist.cpr(cfsim)
  return(list(
        list(hist.cpr[1:cutoff.mo,cpr01]),
        list(hist.cpr[1:cutoff.mo,cpr03]),
        list(hist.cpr[1:cutoff.mo,cpr06]),
        list(hist.cpr[1:cutoff.mo,cpr12]))
  )
}# sim.trials()
```

```{r run.pool.simulation.trials}
cutoff.mo<-60 # only include prepay from month 1:cutoff.mo
trials<-1000
results<-data.table(trial=1:trials)
set.seed(666)
time<-system.time(
  results[,c("cpr01","cpr03","cpr06","cpr12"):=sim.trials(cutoff.mo),by="trial"]
)

title<-sprintf("%d Loan Pool Simulation, %d Trials",loan.count,trials)
print.time.stats(time,title=title)

quants=c(0.005,0.025,0.05,0.95,0.975,0.995)
...
plot.cpr.dist<-function(cprs,title=NA,desc=NA,lo=NA,hi=NA){
  ...
  show.breaks<-seq(from=0.0,to=show.max.cpr,by=0.01)
...
title<-sprintf("Loan Simulation @ %3.1f%% CPR\n%d loans/pool, %d trials, Months 1-%d mo",
               scalar.cpr*100,loan.count,trials,cutoff.mo)
...
#### Historical CPR Function

```{r calc.hist.cpr.f,echo=TRUE,eval=FALSE}
```

#### Simulated CPR Statistics

```{r run.pool.simulation.trials,echo=TRUE,eval=FALSE}
```

```{r show.pool.simulation.trials,echo=TRUE,eval=FALSE}
```
