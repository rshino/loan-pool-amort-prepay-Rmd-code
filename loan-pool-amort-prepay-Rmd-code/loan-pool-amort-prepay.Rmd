...
At the end of month 360, we have:
...
\end{aligned}
$$
To derive *m*, we observe that terms with *m* as coefficient represent a geometric series, which is nominally specified:


$$
\begin{aligned}
S_n = a r^0 + a r^1 + ... +a r^n = \sum_{i=0}^n a r^k
\end{aligned}
$$
...
$$
\begin{aligned}
S_n = m (\frac{1-(1+r_m)^{n+1}}{1-(1+r_m)})
\end{aligned}
$$
Replacing the 2nd to nth terms:

$$
\begin{aligned}
B_{360} & =B_0 (1+ r_m)^{360} -  S_{359}\\
B_{360} & =B_0 (1+ r_m)^{360} -  m (\frac{1-(1+r_m)^{360}}{1-(1+r_m)})\\
\end{aligned}
$$
...
$$
\begin{aligned}
0 & =B_0 (1+ r_m)^{360} -  m (\frac{1-(1+r_m)^{360}}{1-(1+r_m)})\\
0 & =B_0 (1+ r_m)^{360} -  m (\frac{1-(1+r_m)^{360}}{1-(1+r_m)})\\
m (\frac{1-(1+r_m)^{360}}{1-(1+r_m)}) & =B_0 (1+ r_m)^{360} \\
m & =B_0 (1+ r_m)^{360} (\frac{1-(1+r_m)}{1-(1+r_m)^{360}}) \\
m & =B_0 (1+ r_m)^{360} (\frac{r_m}{(1+r_m)^{360}-1}) \\
m & =B_0 r_m \frac {(1+ r_m)^{360}}{(1+r_m)^{360}-1} \\
...
simulation example, loans may have heterogeneous properties (different coupons, sizes, etc).
...
From loan and pool cashflows, observed CPRs may be calculated from pool balances and
...
```{r pool.hist}
#| fig.cap='Pool Prepayment (paydown using MBS Convention)'
...
```{r sim.cpr}
#| fig.cap='Pool Prepayment (paydown by Loan Simulation)'
...
#| fig.cap=c('Small Pool Simulation','Medium Pool Simulation','Large Pool Simulation')
...
```{r alt.sim.cpr}
#| fig.cap=c('Small Pool Historical CPR','Medium Pool Historical CPR','Large Pool Historical CPR')
...
```{r medbig.pool.simulation.trials,results='asis'}
