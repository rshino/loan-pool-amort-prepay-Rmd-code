...
# in data.table
sim.trials<-function(cutoff.mo,loan.count){
  cfsim<-simulate.loan.pool.f(orig.term,note.rate,orig.bal,scalar.cpr,loan.count)
...
```{r run.pool.simulatioxxx,eval=FALSE}
...
time<-system.time(
  results[,c("cpr01","cpr03","cpr06","cpr12"):=
            sim.trials(cutoff.mo,loan.count),by="trial"]
)
...
pool.simulation<-function(trials,cutoff.mo,loan.count){
  results<-data.table(trial=1:trials)
  time<-system.time(
    results[,c("cpr01","cpr03","cpr06","cpr12"):=
              sim.trials(cutoff.mo,loan.count),by="trial"]
...
sim<-pool.simulation(trials,cutoff.mo,loan.count=lc)
...
```{r show.pool.sim.results,results='asis',eval=FALSE}
show.pool.sim.results(sim=sim)
```
...
```{r show.pool.simulation.trialsXX,eval=FALSE}
...
# Listing of Code used in this Document
